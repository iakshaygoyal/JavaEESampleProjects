...
public class ElaichiTeaManager implements BeverageManager {
	...
	public Beverage prepare(BeverageType beverageType) throws Exception {
		try {
			...
			Ingredient elaichiSyrup = ingredientStock.takeIngredient(IngredientType.ELAICHI_SYRUP, 100);
			return new Beverage(beverageType, Arrays.asList(hotMilk, hotWater, sugarSyrup, teaLeaves, elaichiSyrup));
		} catch (Exception e) {
